...
let cachedData=localStorage.getItem('covidCases') ;
let lastCall=JSON.parse(localStorage.getItem("lastFunctCall"))
if(cachedData==null || dayjs(lastCall)<dayjs()){

    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          localStorage.setItem("covidCases", this.responseText);
          
        } // end if
      }; // end xhttp.onreadystatechange = function()
    localStorage.setItem("lastFunctCall", JSON.stringify(dayjs().add(24, "h")))
...
function loadContent() {
  ...
  covidJsObj = JSON.parse(localStorage.getItem("covidCases"));

//   Call function to process the data
  newArray=processData(covidJsObj.Countries);
  ...
  chartData.data.datasets[2].backgroundColor = "rgba(161, 214, 105,1)"; // blue
  chartData.data.datasets[0].label = 'Total';
  chartData.data.datasets[1].label = 'Total Deaths';
  chartData.data.datasets[2].label = 'TotalCases per 100000';
  ...
  chartData.options.title.text = `Covid-19 Hotspots ( ${dayjs().format("MMMM DD, YYYY")} )`;
...
function processData(data){
  ...
  console.log(data)
  data.map(each=>{
    let country=populations[each.Slug]
    if (country && each.TotalDeaths >= 50000){
     
      newArray.push({
        "Slug": "\"" + each.Slug + "\"",
        "TotalConfirmed": each.TotalConfirmed,
        "TotalDeaths" : each.TotalDeaths, 
        "Population": populations[each.Slug], 
        "TotalConfirmedPer100000" : Math.round(100000 * each.TotalConfirmed / populations[each.Slug])
      })
    }
  })
//Return the orderered array
  return _.orderBy(newArray, "TotalConfirmedPer100000", "desc");
}